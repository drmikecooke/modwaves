...
from .parameters import Hmx_d,Hmx_0,d_L,Hmx_L
...
def wave(N,H,L_d=0,scaler=0,A0=None):
    ...
    if not len(A0)==N+2:
        A0=np.insert(A0,-2,np.zeros(N+2-len(A0))) # zero pad A0
...
def wave_base(N,H,L_d=0,fs=lambda h:None): 
